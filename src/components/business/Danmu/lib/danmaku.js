import { SPEED_ARG, DEFAULT_TRACK_SIZE, DEFAULT_RENDER_INTERVAL, defaultDanmakuData } from './constants'

import { getTranslateX } from './dom'
...
export default class Danmaku {
  ...
  _addToTrack(data) {
    ...
    for (let i = 0; i < this._tracks.length; i++) {
      ...
      if (track.length) {
        ...
        // 如果不会重叠，则可以使用当前轨道
        if (distance > lastItem.width && (data.rollSpeed <= lastItem.rollSpeed || (distance - lastItem.width) / (data.rollSpeed - lastItem.rollSpeed) > (this._totalWidth + lastItem.width - distance) / lastItem.rollSpeed)) {
          y.push(i)
        } else {
          y = []
        }
  ...
  _renderToDOM() {
    ...
    if (!node) {
      ...
      data.rollTime = data.rollTime || Math.floor(data.totalDistance * SPEED_ARG * (Math.random() * 0.3 + 0.7))
