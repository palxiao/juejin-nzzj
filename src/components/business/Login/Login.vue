  ...
  <div v-show="show" class="login">
    <div class="test-wrapper-3" style="width: 480px">
      <div class="newsletter">
        <h2 class="ZK">{{ loading ? "信号接收中稍等一会..." : "请输入你的掘金 ID" }}</h2>
        ...
        <form action="" style="margin-bottom: 1.2rem">
          <div class="c-input-group">
            ...
            <input
              ...
              class="c-input"
              id="newsletter"
              placeholder="主页链接URL的末尾数字"
            />
          </div>
          <button @click="done" :class="['c-button','ZK', { loading }]">
            {{ loading ? "请稍候 .." : "查看报告" }}
          </button>
        </form>

        <div class="flex-layout" style="color: #888">
          <label for="scales">滚动时播放 BGM (背景音乐) </label>
          <input style="margin-left: .7rem;" type="checkbox" @change="bgmChange" v-model="playBGM" />
        </div>
      </div>
    </div>
  </div>
  ...
  setup(props, context) {
    ...
    const done = async (e: any) => {
      ...
      const { data } = await axios.get(
        `https://juejin.palxp.com/user?id=${id.value}`
      );
    ...
    const bgmChange = () => {
      baseStore.setBGMPlay(playBGM.value);
    }
